...
service.create(plugin.title, plugin.id + ":start", 'PLAYFREE', true, logo);
...
function showPlaylist(page) {
    ...
    if (!playlist || !playlist.toString()) 
        popup.notify('PLAYFREE ES GRATIS - NO PAGUES POR EL', 5);
...
function readAndParseM3U(page, pl, m3u) {
    ...
    page.loading = true;
    if (!m3u) {
        page.metadata.title = 'Cargando contenido de PLAYFREE...';
    ...
    for (var i = 0; i < m3u.length; i++) {
        page.metadata.title = 'Cargando contenido PLAYFREE, Espere... ' + i + ' of ' + m3u.length;
        line = m3u[i].trim();
...
function addItem(page, url, title, icon, description, genre, epgForTitle, headers) {
    ...
    if (!linkUrl) {
        ...
        var item = page.appendItem(link, type, {
			            title: new RichText((title ? coloredStr('', red) + title : '') + epgForTitle),
            icon: icon ? icon : null,
            genre: genre,
            description: new RichText((title ? coloredStr('El contenido de este plugin es gratis, Y su redistribucion y modificacion esta prohibida : ', blue) + title : '') +
                (description ? 'El contenido de este plugin es gratis,  Y su redistribucion y modificacion esta prohibida:' + description : ''))
        });
...
new page.Route(plugin.id + ":loadOnePlaylist", function(page) {
    ...
    page.loading = true;
    page.metadata.title = 'Cargando el Contenido de PLAYFREE...';
...
new page.Route(plugin.id + ":start", function(page) {
    ...
    page.appendItem("", "separator", {
        title: '_-_ P S 3 U L T I M A T E . C  O M _-_'
    });
   page.appendItem("", "separator", {
        title: ' '
    });
    ...
    page.options.createAction('rmPlaylist', 'Borrar Lista...', function() {
        ...
        if (!i) popup.notify('No Hay listas para Borrar.', 2);
    });
    ...
    if (!service.disableSampleList) {
			page.model.contents = 'grid';
		var item = page.appendItem('m3u:https://archive.org/download/LISTAS2022/accion.m3u:ACCION', "list", {
            title: 'ACCION',
			icon: 'http://1.bp.blogspot.com/-R4d1YFTsPVU/Xll2ISJoh4I/AAAAAAAABwU/1f4NG9RxbIw4_IB40663fpN9Ny4kPGebACLcBGAsYHQ/s1600/ACCION.jpg'
        });
    }

    if (!service.disableSampleXMLList) {
			page.model.contents = 'grid';
        var item = page.appendItem('m3u:%68%74%74%70%3a%2f%2f%70%73%33%75%6c%74%69%6d%61%74%65.%63%6f%6d%2f%6c%69%73%74%61%73%2f%70%61%73%74%65.%70%68%70?%64%6f%77%6e%6c%6f%61%64&%69%64%3d12?.m3u:%50%53%33%55%6c%74%69%6d%61%74%65', "directory", {
            title: '>SERIES<',
			icon: 'http://jomiloma.com/p3f/02.jpg'
        });
    }
 if (!service.disableSampleXMLList) {
	 	page.model.contents = 'grid';
        var item = page.appendItem('m3u:%68%74%74%70%3a%2f%2f%70%73%33%75%6c%74%69%6d%61%74%65.%63%6f%6d%2f%6c%69%73%74%61%73%2f%70%61%73%74%65.%70%68%70?%64%6f%77%6e%6c%6f%61%64&%69%64%3d24?.m3u:%50%53%33%55%6c%74%69%6d%61%74%65', "directory", {
            title: '>MUSICA<',
			icon: 'http://jomiloma.com/p3f/03.jpg'
        });
    }    
	if (!service.disableSampleList) {
			page.model.contents = 'grid';
        var item = page.appendItem('m3u:%68%74%74%70%3a%2f%2f%70%73%33%75%6c%74%69%6d%61%74%65.%63%6f%6d%2f%6c%69%73%74%61%73%2f%70%61%73%74%65.%70%68%70?%64%6f%77%6e%6c%6f%61%64&%69%64%3d25?.m3u:%50%53%33%55%6c%74%69%6d%61%74%65', "directory", {
            title: '>IPTV<',
			icon: 'http://jomiloma.com/p3f/04.jpg'
        });
    }
    if (!service.disableProviderList) {
        ...
        page.appendItem(plugin.id + ":onePlaylistStart", "directory", {
            ...
            icon: 'https://archive.org/download/PLAYFREE_img/01.jpg'
        });
        page.appendItem(plugin.id + ":streamliveStart", "directory", {
            title: "StreamLive.to",
            icon: 'https://archive.org/download/PLAYFREE_img/02.jpg'
        });
        page.appendItem(plugin.id + ":tivixStart", "directory", {
            title: "Tivix.co",
            icon: 'https://archive.org/download/PLAYFREE_img/03.jpg'
        });
        page.appendItem(plugin.id + ":youtvStart", "directory", {
            title: "Youtv.com.ua",
            icon: 'https://archive.org/download/PLAYFREE_img/04.jpg'
        });
        page.appendItem(plugin.id + ":goAtDeeStart", "directory", {
            title: "goATDee.Net",
			con: 'https://archive.org/download/PLAYFREE_img/05.jpg'
        });
	    ...
	    page.appendItem("", "separator", {
        title: 'Creado por BRIAN-JESHUA y ZukoMRC'
                });
	    page.appendItem("", "separator", {
        title: 'Para la comunidad ZUKOSTORE'
                });	
		 page.appendItem("", "separator", {
        title: ''
			    });		
